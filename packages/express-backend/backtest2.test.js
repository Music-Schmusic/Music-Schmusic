...
test('/accountrecovery returns 401 email mismatch', async () => {
  ...
  jest
    .spyOn(dbrequests, 'getAccount')
    .mockImplementationOnce((username) => mockUser);
...
test('/accountrecovery returns recovery token successfully', async () => {
  ...
  expect(dbrequests.addRecoveryToken).toHaveBeenCalledWith(
  ...
  expect(mailer.sendEmail).toHaveBeenCalledWith(
    email,
    expect.stringContaining('Click here to recover account:'),
    'Password Recovery'
  );
});
...
test('/resetvalidation returns successful 200 response', async () => {
  ...
  const getRecov = jest
    .spyOn(dbrequests, 'getRecoveryToken')
    .mockImplementationOnce((token) => mockRecord);
  const res = await request(app)
    .post('/resetvalidation')
    .set('Cookie', [`CRSFtoken=${CRSFtoken}`])
    .send(mocReq);
...
test('/resetpassword returns 200 successful response', async () => {
  ...
  const account = {
    ...
    email: 'test@test.com',
    password: pass,
    spotifyAccessToken: 'abc',
    spotifyRefreshToken: 'abc',
    spotifyTokenExpiresAt: Date(1985, 1, 1),
  };
  const mockReq = {
    p1: pass,
    p2: pass,
  ...
  expect(res.status).toBe(200);
  expect(res.text).toBe('Password successfully updated');
  expect(resetPass).toHaveBeenCalledTimes(1);
  expect(resetPass).toHaveBeenCalledWith(user, pass);
});

test('/resetpassword returns 400 password mismatch', async () => {
  jest.mock('./Functionality/account.js', () => ({
    resetPassword: jest.fn(),
  }));
  const user = 'user';
  ...
  expect(resetPass).toHaveBeenCalledTimes(0);
});
